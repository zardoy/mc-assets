...
export default function worldBlockProvider(blockstatesModels: any, blocksAtlas: any, version: string) {
    ...
    const transformModel = <T extends BlockModel | ResolveModelReturnType[number][number]>(model: T, block: Omit<QueriedBlock, 'stateId'>, elementsProcessingIssues?: string[]) => {
    ...
    return {
        getAllResolvedModels0_1(block: Omit<QueriedBlock, 'stateId'>, fallbackVariant = false, possibleIssues: string[] = [], matchedModels: string[] = [], matchedConditions: string[] = [], partialElementsProcessing = false) {
            assetsParser.issues = []
